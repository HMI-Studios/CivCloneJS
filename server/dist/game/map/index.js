...
class Map {
    ...
    tileUpdate(coords) {
        ...
        const tile = this.getTileOrThrow(coords);
        this.updates.push((civID) => ['tileUpdate', [coords, this.getTileDataByCiv(civID, tile)]]);
    }
    ...
    updateImprovementLinks(world, tile, coords, improvement) {
        ...
        for (const pos in posDistances) {
            const otherTile = this.tiles[Number(pos)];
            if (!otherTile ||
