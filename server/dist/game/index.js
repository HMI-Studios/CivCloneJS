...
class Game {
    constructor(args) {
        ...
        while (!world && !(tries > maxTries)) {
            tries++;
            try {
                world = new world_1.World([generator.generate(), playerCount]);
            }
            catch (err) {
        ...
        if (!world)
            throw new error_1.MapError('Map failed to generate and the error was not caught.');
        this.world = world;
        this.players = {};
        this.playerCount = playerCount;
        this.metaData = {
    ...
    static import(data) {
        const world = world_1.World.import(data.world);
        const players = {};
        for (const playerName in data.players) {
            const playerData = data.players[playerName];
            players[playerName] = player_1.Player.import(playerData);
        }
